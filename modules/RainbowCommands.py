...
class RainbowCommands:
    ...
    async def r_makerole(self, ctx):
        ...
        if ctx.message.author.server_permissions.manage_roles is True:
            ...
            if existence_check is None:
                ...
                await self.client.say(DBtext[14])
            else:
                msg = await self.client.say(DBtext[13])
                await bot.clear_last_selfmessage(self.client, msg, msg.channel)
        else:
            await self.client.sat(DBtext[0])
    ...
    async def r_giverole(self, ctx, targetID=''):
        ...
        if ctx.message.author.server_permissions.manage_roles is True:
            if targetID == '':
                ...
                user = await self.client.get_user_info(bot.clrUID(targetID))
    ...
    async def r_start(self, ctx):
        ...
        if ctx.message.author.server_permissions.ban_members is True:
            await self.client.say(DBtext[29])
            ...
            if msg is not None:
                if msg.content == 'Yes' or msg.content == 'Y' or msg.content == 'yes' or msg.content == 'y':
                    await self.client.say(ctx.message.author.mention + DBtext[15])
                    ...
                    while rainbow_flag:
                        if rainbow_flag is True:
                            ...
                            for i in range(len(colorlist1)):
                                await self.client.edit_role(sName, rName, color=discord.Color(colorlist1[i]))
                                await asyncio.sleep(cd)
                        else:
                            return
                else:
                    msg = await self.client.say(DBtext[30])
                    await bot.clear_last_selfmessage(self.client, msg, msg.channel)
            else:
                msg = await self.client.say(DBtext[28])
                await bot.clear_last_selfmessage(self.client, msg, msg.channel)
        else:
            msg = await self.client.say(DBtext[18])
            await bot.clear_last_selfmessage(self.client, msg, msg.channel)
    ...
    async def r_stop(self, ctx):
        ...
        global rainbow_flag
        rainbow_flag = False
        await self.client.say(DBtext[16])
